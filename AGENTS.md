...
- Delegate only through OMP. NEVER start a Codex session, invoke the `codex` CLI, or use a Codex agent type as a fallback. If required OMP support cannot start, report the blocked dependency.
