...
The expanded OMP production cohort found useful challenger signals without identifying a new general default, but it ran with passive Advisor enabled and therefore measures the treated system rather than each implementer in isolation. **Luna max** led weighted efficiency on the exploratory distributed-systems task, while **Kimi K3**, **GLM 5.2 max**, and **Opus 5** produced competitive artifacts in some roles. Strict advisor-disabled K3/Opus confirmation and a matched GLM/Sol native-editor check are reported separately; only those strict results support July pure-route claims. Keep automatic advisor routes **disabled** and invoke independent review explicitly.
...
### Expanded OMP exploratory candidate cohort

The July OMP cohort added Kimi K3, Opus 5 high, GLM 5.2 max, Luna max, and Inkling, then repeated top incumbent routes on production-shaped distributed-systems, native-editor, native offline/data, and collection-query execution tasks. The implementation sessions had passive Advisor enabled, and some judge sessions were also advised. The retained rows therefore measure an advisor-on treated system. They support screening and follow-up allocation, but cannot assign pure Default or SLOW routes to an implementer. Strict confirmation disables Advisor, extensions, skills, rules, LSP, xdev, Auto QA, and undeclared discovery providers; it records the observed tool surface and is reported separately.
...
| GLM 5.2 max · Baseten | 73.00 | $6.930 | 2,600.8 s | 7/8 | Carry with repair; validation failed |
| Inkling · Baseten | 7.67 | $3.260 | 787.5 s | 8/8 | Redrive; invalid source artifact |

**Within the advisor-on exploratory treatment:** Luna max produced Sol-range quality at less than one third of Sol's observed route cost and led the policy-weighted metric. Kimi K3 led raw quality but took nearly three times Terra's model-session time. Opus 5 was also strong but cost almost five times Terra. GLM's candidate-native test failure is a true artifact defect, not harness noncompliance. This task earned strict follow-up; it does not establish implementer-alone quality or justify changing the shared default.
...
### Strict advisor-disabled K3 and Opus confirmation

The pure-route confirmation uses OMP with Advisor, extensions, skills, rules, LSP, xdev, Auto QA, and undeclared discovery providers disabled. It records the observed tool surface, keeps candidate sessions uncapped by elapsed time, separates model-session time from harness time, and applies a three-model blind panel followed by source reconciliation. The completed matched results are:

| Production-shaped workload | K3 quality / weighted | Opus quality / weighted | Source-reconciled decision |
|---|---:|---:|---|
| Modern client | 82.7 / **68.7** | **86.0** / 64.2 | Same quality range; K3 wins value |
| Native editor | 71.7 / 53.6 | 71.7 / **61.3** | Tie on quality; Opus is 2.29× faster |
| Android maintenance | Provider-censored | **72.7 / 63.0** | K3 excluded after 21 stream stalls, six 429s, and a socket close |
| Collection-query execution | 78.7 / 60.0 | **81.7 / 67.9** | Opus; K3 is 2.61× slower after removing 429 waits |
| Desktop native | 54.0 / 41.7 | **86.0 / 59.3** | Opus; large source-quality gap survives 429 normalization |
| Native offline/data | 1.0 / 40.0 · Redrive | **66.0 / 20.3** | Opus; K3 delivered no patch |
| Public API | 0.3 / 40.0 · Redrive | **58.3 / 25.9** | Opus; K3 delivered no patch |
| Mixed monetization | **62.0** / 45.7 | 57.7 / **45.9** | Same quality range; K3 is cheaper and higher-quality, Opus is 2.34× faster |

The offline and Public API rows demonstrate why arithmetic is subordinate to validity and handoff. K3's zero-patch routes were cheapest and fastest, so the descriptive weighted formula awards them 40 efficiency points; they remain ineligible to win. Across the five matched lanes where both routes delivered carryable artifacts, K3 led pure Quality:Value five times because it was cheaper, while Opus led 429-normalized weighted quality/cost/speed four times because it was usually faster and higher-quality. Mean weighted score was **53.9 for K3** and **59.7 for Opus**; mean pure Quality:Value was **36.3 for K3** and **27.7 for Opus**. Restricting to the four same-quality-range pairs, K3 was **23.9% cheaper**, **2.34× slower** after removing 429 waits, and **0.5 quality points lower** in aggregate; Opus led blended value **59.8 to 57.0**, while K3 led pure Quality:Value **41.5 to 30.7**. K3's provider-censored Android and agent-runtime attempts are excluded from model-quality and speed aggregates.

Strict K3 also reproduced a high-quality distributed-systems result at **87.3**, but no advisor-disabled Opus control exists for that workload. Opus completed a strict **70.7** agent-runtime artifact without a matched usable K3 result. The public-API K3 route produced no patch after emitting literal tool-protocol text without a provider stall. K3 agent-runtime is provider-censored: after 259 completed responses, Baseten returned 66 rate-limit errors and 11 stream stalls and the route ended with no patch. It is retained as launch-day serving evidence, not a K3 semantic-quality result. On mixed monetization, K3's 62.0 artifact beat Opus by 4.3 quality points and cost 22.4% less, but destructive test deletion, unsafe billing failure state, incomplete localization and plans-page surfaces, and silent broad coupon behavior require repair.

**Routing interpretation:** K3 has a real cost signal on successful work, but the strict cohort does not support Default, SLOW, SMOL, or unattended high-volume assignment. Opus is the more reliable premium challenger and often buys enough speed to win the blended metric, but it has not displaced Sol on value. Provider-reported throughput—K3 about 31 TPS, Sol about 40, Opus about 50–60, Terra about 55–60, GLM 5.2 about 54, and Luna about 60—is deployment context rather than measured benchmark evidence. Raw time, 429-normalized time, and a separate provider-incident sensitivity remain auditable; only 429 time is removed from the primary score. Inkling's advisor-on three-workload screen justifies stopping discretionary expansion; it does not establish a clean Inkling-alone ranking or support any OMP role assignment.

### Strict advisor-disabled GLM and Sol confirmation

The native-editor follow-up directly compared GLM 5.2 max with the retained Sol high control under the strict tool surface. GLM scored **51.7** versus Sol's **63.7**, cost **$1.974** versus **$1.394**, and took **568.9 seconds** versus **223.6 seconds**. GLM's weighted quality/cost/speed score was **31.8** versus Sol's **55.5**. All three judges independently identified malformed Swift documentation that prevents compilation, plus composing behavior that can lose the incoming remote value. The validation process itself exited without a verdict, so the compile finding comes from source reconciliation rather than a misclassified harness failure.

This strict result does not support GLM for the Sol premium route: GLM was **12.0 quality points lower, 41.6% more expensive, and 2.54× slower**. Across the broader three advisor-on overlaps, GLM averaged 71.0 quality versus Sol's 77.8 while costing 22.6% less and taking 1.97× as long. The apparent aggregate cost advantage did not survive the strict matched check. GLM remains an explicit experimental route, not a shared default or premium replacement. Historical GLM rows were repriced with the current OMP catalog's **$0.14/M cached-input rate**; the raw results remain immutable and carry correction metadata.
...
| GLM 5.2 max · Baseten | $1.40 / $0.14 / $4.40 | **524,288 current catalog** | Advisor-on July OMP: 77 editor, 63 offline/data, and 73 distributed-systems means; strict editor follow-up fell to 51.7 with a compile-blocking defect |
...
| Bounded implementer | No shared open-weight default | Kimi K3 earned matched follow-up on distributed systems and iOS; strict GLM max confirmation did not beat Sol. Neither has cross-domain value evidence for default promotion |
